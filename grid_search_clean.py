...
def complete_grid_search(df0, df_labels, parameters, results):    
    if feature_extraction == "AE": 
        df = pd.concat([df0,df0_imp_features], axis =1)   
        ...
        for s in parameters['min_cluster_size']:
            for t in parameters['min_samples']:
                for u in parameters['epsilon']:
                    ...
                    print("epsilon, min_cluster_size, min_samples:", u, s, t)
            ...
            for s in parameters['min_cluster_size']:
                    for t in parameters['min_samples']:
                        for u in parameters['epsilon']:
                            ...
                            noise_count = df_hdbscan['cluster'].value_counts()[-1]
                            df_hdbscan = df_hdbscan.loc[df_hdbscan['cluster'] > -1] 
                            df_hdbscan.to_csv("C:/Users/arienne.calonge/Py/Acoustics_paper/results_grid_search/"+str(len(results)-1)+'_results_grid_search.csv')
...
parameters_AE_cropped = {'alpha':{0.01, 0.05, 0.08},'min_cluster_size':[5,8,10,12], 'min_samples':[3,4,5], 'epsilon':[0.2, 0.5, 0.8]} 
parameters_AE_standard = {'alpha':{0.01, 0.02, 0.03},'min_cluster_size':[5,8,10,12], 'min_samples':[3,4,5], 'epsilon':[0.2, 0.5, 0.8]} 
...
for file in os.listdir(directory):
    raw = os.path.join(directory, file)
...
results.to_csv("C:/Users/arienne.calonge/Py/Acoustics_paper/results_grid_search/results_grid_search.csv")
...
#export csv
#df_hdbscan.to_csv("C:/Users/arienne.calonge/Py/Acoustics_paper/Aves_max_146_grid_search.csv")
